...
use app\modules\formBuilder\models\DetailesForm;
use app\modules\formBuilder\models\Enumeration;
...
<table id="w0" class="table table-striped table-bordered detail-view">
    ...
    $string =null;
    $listLable = DetailesForm::getItems();
    $counter = 0 ;

    foreach ($list as $key=>$value)
    {
        $check = Enumeration::getEnumListByType($key);

        if ($check)
        {
            $string .= '<tr>';
            $string .= '<th>'. $listLable[$counter]->lable .'</th>';
            $string .= '<td>'. Enumeration::getEnumTitileById($value) .'</td>';
            $string .= '</tr>';
        }
        else
        {
